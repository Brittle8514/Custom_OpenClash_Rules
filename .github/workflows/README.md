...
| 文件名 | 描述 | 触发条件 |
| :--- | :--- | :--- |
| **[auto-backup-wiki.yml](auto-backup-wiki.yml)** | 自动备份 GitHub Wiki 内容到仓库的 `wiki/` 目录，并处理链接替换 | 每天 0 点 / 手动触发 |
| **[auto-generate-rules.yml](auto-generate-rules.yml)** | 从 `.list` 规则文件自动生成 `.yaml` 和 `.mrs` 格式的规则集 | `rule/*.list` 变更 / 手动触发 |
| **[auto-update-game-cdn.yml](auto-update-game-cdn.yml)** | 从 v2fly 上游自动更新 `Game_Download_CDN.list` 规则文件 | 每 12 小时 / 手动触发 |
